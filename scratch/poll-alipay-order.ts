...
async function main() {
  ...
  if (!fs.existsSync(statePath)) {
    console.error('未找到 order_state.json，请先运行 create-alipay-order.ts');
    process.exit(1);
  }

  const state = JSON.parse(fs.readFileSync(statePath, 'utf8'));
  const { orderId, aoid, authCookie } = state;

  console.log(`开始轮询订单. 订单 ID: ${orderId}, BufPay AOID: ${aoid}`);
  ...
  while (Date.now() - startTime < maxWaitMs) {
    ...
    process.stdout.write(`\r⌛ 已等 ${elapsed}秒 / 官方: ${officialStatus} / 本地: ${localStatus}`);
