...
async function main() {
  ...
  assert(typeof codeA1 === 'string' && codeA1.length === 8, `获取绑定码 A1: ${codeA1}`);
  ...
  // 4.1 查询 Web 绑定状态，确认状态为 bound，微信号脱敏
  ...
  assert(bodyStatusA.masked_external_id === maskWechatId('test_wxid_A'), `微信号脱敏匹配: ${bodyStatusA.masked_external_id}`);
  ...
  // 6. 微信号冲突防重测试 (微信 ID 只能绑定一个 Profile)
  console.log('\n6. 验证微信号唯一性冲突防范 (一微多户拦截)');
  ...
  assert(typeof codeB1 === 'string', `获取绑定码 B1: ${codeB1}`);

  // 6.2 尝试将用户 B 的绑定码 B1 绑定到已与用户 A 绑定的 'test_wxid_A' 微信上
  ...
  const resConflictWechat = await fetch(webhookUrl, {
    ...
    body: JSON.stringify({
      ...
      hermes_user_id: 'test_wxid_A', // 冲突微信号
      text: codeB1,
      timestamp: Date.now()
    })
  });
  ...
  // 7. 用户重复绑定微信号拦截测试 (一户多微拦截)
  console.log('\n7. 验证用户重复绑定拦截 (一户多微拦截)');
  
  // 7.1 已绑定的用户 A 重新生成绑定码 A3
  ...
  // 7.2 尝试将已绑定的用户 A 绑定到新微信 'test_wxid_C' 上
  ...
  const resBindB = await fetch(webhookUrl, {
    ...
    body: JSON.stringify({
      ...
      hermes_user_id: 'test_wxid_B', // 干净的微信号
      text: codeB1,
      timestamp: Date.now()
    })
  });
